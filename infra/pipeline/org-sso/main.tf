...
locals {
  ...
  development_account_id = local.development_account_exists ? local.existing_development_account[0].id : (var.development_account_id != "" ? var.development_account_id : try(aws_organizations_account.development["create"].id, ""))
  production_account_id  = local.production_account_exists ? local.existing_production_account[0].id : (var.production_account_id != "" ? var.production_account_id : try(aws_organizations_account.production["create"].id, ""))
}
